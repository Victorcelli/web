...
function detalle_carrito(listaDeEmbutidos, i, j, k)
{
	...
	if (encontrado)
	{
		matriz[x][0]++;
       	...
       	matriz[x][2]= fila_compra[2] * matriz[x][0];
	...
	while(x < total_filas)
	{
		...
		tbody.appendChild(filaXProducto);
...
//*************************************************************************************************************************************************************
function solicitar_compra(matriz)
{
	iniciar();
	
	http_request.onreadystatechange = "";
	
	http_request.open("POST", "../PHP/datos_carrito.php");
	...
	http_request.setRequestHeader("Content-type","application/x-www-form-urlencoded");
        ...
        datos = JSON.stringify({carrito: matriz})
        
	http_request.send("carrito=" + datos);	
