...
//*************************************************************************************************************************************************************
function borrar_formulario_alta()
{
	var nick = document.getElementById("inputTextNick").value; //  document.formNuevoUser.inputTextNick.value  
	var pas = document.getElementById("inputTextPas").value;
	var nif = document.getElementById("inputTextNIF").value;
...
// OPCION ENVIAR EMAIL - ENVIA CREDENCIALES A USUARIO SI ESTÁ EN LA BD
//*************************************************************************************************************************************************************
...
function recibe_sesion()
{
	if(http_request.readyState == 4 && http_request.status == 200)
	{
		...
		var divRegistroUser = document.getElementById("divRegistroUser");
                var aNombreSesion = document.getElementById("aNombreSesion");
		...
		else
		{
			...
			divNombreSesion.style.display = "inline-block";
                        ...
                        aNombreSesion.innerHTML = datosPHP;
		}
	}
}
//*************************************************************************************************************************************************************
...
//	SOLICITA EDITAR DATOS
//*************************************************************************************************************************************************************
function solicitar_editar()
{
    iniciar();
    
    http_request.onreadystatechange = recibe_respuesta_editar;
    
    http_request.open("POST", "../PHP/editar_datos.php", true);

    http_request.setRequestHeader("Content-type","application/x-www-form-urlencoded");

    var select_datos = document.getElementsByClassName("inpTextNuevo");
    var datos = [];
    
    for (var i = 0; i < select_datos.length; i++){
        datos[i] = select_datos[i].id + "=" + select_datos[i].value ;
    }
    
    var datos_http= datos.join("&");

    http_request.send(datos_http);

}
//*************************************************************************************************************************************************************


// RESPUESTA SERVIDOR AL EDITAR DATOS
//*************************************************************************************************************************************************************
function recibe_respuesta_editar()
{
   if(http_request.readyState == 4 && http_request.status == 200)
	{
            var datosPHP = http_request.responseText;
            ...
            {
                alert("Sus datos no se han actualizado");
            }
        }
}
//*************************************************************************************************************************************************************
